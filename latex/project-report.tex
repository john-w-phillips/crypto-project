...
number possible. We explore in depth one such instance, a modification
made to the OpenSSL package distributed in debian that resulted in
only 32,768 possible keys that could be generated. We also explore
similarities to heartbleed. Finally, we investigate methods that have
been put in place to prevent such bugs from happening in the future.
\end{abstract}
...
lines of code -- and got the go-ahead (See Fig.\ref{email}).
...
number generator could only really have 32,768 possible outputs from
...
Eventually on the attacker machine you will get the output:
...
Fig.\ref{coverage-demo} for an example of what the coverage looks
