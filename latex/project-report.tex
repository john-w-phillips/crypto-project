...
\label{testfailure}  
\end{figure*}
  Meanwhile, the for the fuzz tests, OpenSSL recommended using LibFuzzer or AFL.
We tried both following their instructions on the README at \url{https://github.com/openssl/openssl/blob/master/fuzz/README.md}.
...
timed the fuzzers for 5 minutes until it crashed or caught something. With
LibFuzzer, we found timeout on \verb|bignum| and \verb|asn1|. We wanted
to further investigate the cause of the timeout, and LibFuzzer was not
being helpful with the warnings. Therefore, we decided to try out
AFL fuzzer as well. Likewise, AFL fuzzer also timed out on
both those fuzzers. However, when we increased the timeout to 5 seconds,
it was successfully able to execute those tests. In conclusion,
we would have to say the fuzz testing were not able to detect any
vulnerabilities. The results are show in Figure 3 for
the timeout failure, and Figure 4 when timeout
gets updated with \verb|-t 5000| option run with AFL (the test ran for almost half
an hour before we decided to conclude nothing was found).
\begin{figure}[h]
  \caption{AFL test timeout for asn1 fuzzer}
  \centering
  \includegraphics[scale=0.4]{asn1-fuzz-failure}
\end{figure}
\begin{figure}[h]
  \caption{AFL test with timeout increased}
  \centering
  \includegraphics[scale=0.5]{asn1-fuzz-timeout}
\end{figure}
