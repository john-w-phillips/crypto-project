...
testing in order to scan the main branch of OpenSSL, but we didn’t get
to doing that. Valgrind is an instrumentation framework that can be
...
checking in general. For example through making more use of the -Wall
flag, cppcheck, and sanitizers. The wall flag would have helped us to
see potential warning messages; Cppcheck for instance is a static
analysis tool for C/C++ code that could have helped us detect
...
would be an area that we can look into. If a security vulnerability
would be possible to implement in an untested area of the code that
would present a significant security risk, making sure that that is
not possible would add to the security of OpenSSL as a whole.
